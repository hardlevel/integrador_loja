      ...
      constructor() {
        super(type);
			...
			async fetchAll(id = null) {
				...
				let data = '';
				if (id) {
					data = await fetchAll(this.config, this.name, id);
				} else {
					data = await fetchAll(this.config, this.name);
				}
				return data;
			};
			...
			async fetch(id = null) {
				...
				let data = '';
				if (id) {
					data = await fetch(this.config, this.name, id);
				} else {
					data = await fetch(this.config, this.name);
				}
				return data;
			};
