...
async function main(config){
	...
	const proccessData = async (controller, data, fields, type, related) => {
		for (const item of data) {
			...
			for (const [key, value] in Object.entries(types)){
				console.log(key, value);
				// console.log(type, types[type].hasOwnProperty('related'))
				if (types[type2].hasOwnProperty('related')) {
					console.log('teste', types[type2].related, type.key);
					if (types[type2].related == type) {
						console.log('teste2', type, item.id);
