...
async function main(config){
	const types = {
		...
		categories: {
			...
			data: {
				...
				related_category: 'item.categoria_pai'
			}
		},
	}
	...
	const init = async (types) => {
		...
		for(const type in types){
			const controller = new (require('../controllers/abstractController').ControllerFactory.createController(type))();
			const data = await controller.fetch();
			await proccessData(controller, data);
		}
	}
	...
	const fetchData = async (type) => {
		...
		//console.log(list);
		await proccessData(type, data);
	}

	const proccessData = async (type, data) => {
		for (const item of data) {
			const mapping = {};
			for (const field in type.data){
				const value = eval(type.data[field]);
        mapping[field] = value;
			}
			await type.controller.save(mapping);
		};
	}
	await init(types);
