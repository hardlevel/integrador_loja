async function main(config){
	...
	const getProductDetails = async () => {
		...
		const products = new (require(config.controller + 'abstractController').ControllerFactory.createController(product, config))();
		const category = types.find(type => type.controller == 'category');
		const categories = new (require(config.controller + 'abstractController').ControllerFactory.createController(category, config))();
		const price = types.find(type => type.controller == 'price');
		const prices = new (require(config.controller + 'abstractController').ControllerFactory.createController(price, config))();
		const image = types.find(type => type.child.controller == 'image');
		const images = new (require(config.controller + 'abstractController').ControllerFactory.createController(image.child.controller, config))();
